...
#Masking matrix to only cluster genes that are not present in every sample. also do it with a cutoff
matrix = matrix.set_index(matrix.columns[0]) 
mask = (matrix == 1).all(axis=1)
maskedMatrix = matrix[~mask]
#maskedMatrixCutOff = 
...
def reducedClusteredHeatmap(data, names):
    """
    Creates a clustered heatmap with the specified formatting and customization.
